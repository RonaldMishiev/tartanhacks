...
from .parsing import process_assembly, parse_mca_output
...
class BoltEngine:
    ...
    def refresh(self):
        ...
        2. Compile to ASM
        3. Clean/Map ASM
        4. Analyze with MCA (if available)
        5. Trigger UI update
        """
        try:
            # 1. Read source
            with open(self.state.source_path, "r") as f:
                self.state.source_code = f.read()
            ...
            if asm_raw:
                # 3. Parse Assembly
                clean_asm, mapping = process_assembly(asm_raw)
            ...
            # 5. Notify the UI if a callback is registered
